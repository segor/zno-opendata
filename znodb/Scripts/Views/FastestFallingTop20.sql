...
EXEC ('DROP VIEW IF EXISTS [zno$(ZnoYear)].[FastestFallingTop20]')
GO

CREATE VIEW [zno$(ZnoYear)].[FastestFallingTop20]
AS
WITH SR_PrevYear AS (
	SELECT  *
	FROM 
...
SELECT TOP 20 SR_PrevYear.TotalRank_D TotalRank_D_PrevYear
	  , [SR_CurrYear].TotalRank_D TotalRank_D_CurYear
...
order by [SR_CurrYear].TotalRank_D + SR_PrevYear.TotalRank_D ASC
