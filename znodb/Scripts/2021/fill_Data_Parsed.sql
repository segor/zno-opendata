...
SELECT 
	...
	dbo.clean_str(UmlTest) AS UmlTest,
	dbo.clean_str(UmlTestStatus) AS UmlTestStatus,
	CAST (dbo.clean_num(UmlBall100) as DECIMAL(4,1)) AS UmlBall100,
	dbo.clean_num(UmlBall12) AS UmlBall12,
	dbo.clean_num(UmlBall) AS UmlBall,
	dbo.clean_str(UmlAdaptScale) AS UmlAdaptScale,
	dbo.clean_str(UmlPTName) AS UmlPTName,
	dbo.clean_str(UmlPTRegName) AS UmlPTRegName,
	dbo.clean_str(UmlPTAreaName) AS UmlPTAreaName,
	dbo.clean_str(UmlPTTerName) AS UmlPTTerName,
	dbo.clean_str(UkrTest) AS UkrTest,
	dbo.clean_str(UkrSubTest) AS UkrSubTest,
	...
	dbo.clean_str(MathDpaLevel) AS MathDpaLevel,
	...
	dbo.clean_str(MathStTest) AS MathStTest,
	dbo.clean_str(MathStLang) AS MathStLang,
	dbo.clean_str(MathStTestStatus) AS MathStTestStatus,
	dbo.clean_num(MathStBall12) AS MathStBall12,
	dbo.clean_num(MathStBall) AS MathStBall,
	dbo.clean_str(MathStPTName) AS MathStPTName,
	dbo.clean_str(MathStPTRegName) AS MathStPTRegName,
	dbo.clean_str(MathStPTAreaName) AS MathStPTAreaName,
	dbo.clean_str(MathStPTTerName) AS MathStPTTerName,
...
UPDATE Data_Parsed
SET       EOName_Adjusted = 'Вільшанська загальноосвітня школа I-III ступенів, Романівський р-н'
WHERE (ZnoYear = 2021) AND (EOName = 'Вільшанська загальноосвітня школа I-III ступенів') AND (EORegName = 'Житомирська область') AND (EOAreaName = 'Романівський район');
